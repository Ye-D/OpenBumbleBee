...
INSTANTIATE_TEST_SUITE_P(
    ...
    [](const testing::TestParamInfo<ActivationTest::ParamType>& p) {
      return fmt::format("{}xPUMA_{}", std::get<0>(p.param),
...
TEST_P(ActivationTest, Seg3Gelu) {
  ...
  spu::mpc::utils::simulate(3, [&](std::shared_ptr<yacl::link::Context> lctx) {
    spu::RuntimeConfig rt_config;
    rt_config.set_protocol(ProtocolKind::ABY3);
    ...
    auto gelu = hal::intrinsic::nn::puma::f_seg3_gelu(ctx, x);
...
TEST_P(ActivationTest, Seg4Silu) {
  ...
  spu::mpc::utils::simulate(3, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    size_t bytes_sent = lctx->GetStats()->sent_bytes;
    auto gelu = hal::intrinsic::nn::puma::f_seg4_silu(ctx, x);
    bytes_sent = lctx->GetStats()->sent_bytes - bytes_sent;
...
TEST_P(ActivationTest, NegExp) {
  ...
  spu::mpc::utils::simulate(3, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    size_t bytes_sent = lctx->GetStats()->sent_bytes;
    auto nexp = hal::intrinsic::nn::puma::f_neg_exp_taylor(ctx, x);
    bytes_sent = lctx->GetStats()->sent_bytes - bytes_sent;
